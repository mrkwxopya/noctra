...
async function bundleReactRuntime() {
  ...
  if (!entry) {
    throw new Error("Could not find packages/react/src/index.ts or index.tsx");
  }

  const outfile = "apps/docs/.tmp-docs-runtime/noctra-react-runtime.mjs";
  ...
  return pathToFileURL(resolve(outfile)).href;
}
...
const reactRuntimeModuleUrl = await bundleReactRuntime();
const reactRuntime = await import(`${reactRuntimeModuleUrl}?v=${Date.now()}`);
...
for (const name of componentNames) {
  ...
  if (!RuntimeComponent) {
    problems.push(`${name}: runtime export missing from bundled @noctra/react source runtime`);
    continue;
  }
