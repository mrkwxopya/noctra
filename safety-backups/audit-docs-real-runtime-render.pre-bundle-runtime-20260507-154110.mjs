...
async function loadReactRuntime() {
  ...
  try {
    candidates.push(appRequire.resolve("@noctra/react"));
  } catch {}
  ...
  for (const candidate of uniqueCandidates) {
    try {
      if (candidate.endsWith(".cjs")) {
        return appRequire(candidate);
      }

      return await import(`${pathToFileURL(candidate).href}?v=${Date.now()}`);
    } catch (error) {
      lastRuntimeError = error;
    }
  }

  throw new Error(`Could not load @noctra/react runtime. Last error: ${lastRuntimeError instanceof Error ? lastRuntimeError.message : String(lastRuntimeError)}`);
}
...
for (const name of componentNames) {
  ...
  if (!RuntimeComponent) {
    problems.push(`${name}: runtime export missing from @noctra/react`);
    continue;
  }
