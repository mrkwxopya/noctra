...
const reportPath = "real-props-styles-foundation-report.md";
...
const componentDocKindFunction = `function getComponentDocKind(slug: string):
  | "button-like"
  | "field"
  | "selection"
  | "choice"
...
if (!page.includes("function getComponentDocKind")) {
  const marker = "function runtimeComponent";
  const index = page.indexOf(marker);

  if (index < 0) {
    throw new Error("runtimeComponent marker not found.");
  }

  page = `${page.slice(0, index)}${componentDocKindFunction}\n\n${page.slice(index)}`;
}
...
export function Demo() {
  return (
    <\${name} opened title="\${label}" onClose={() => {}}>
      Layered content goes here.
    </\${name}>
  );
}
\`;
...
export function Demo() {
  return (
    <\${name} withBorder padding="md" radius="\${state.radius}">
      <strong>\${label}</strong>
      <p>Structured content surface.</p>
    </\${name}>
  );
}
\`;
...
export function Demo() {
  return (
    <\${name}>
      ...
      <tbody>
        <tr><td>Noctra UI</td><td>Ready</td></tr>
      </tbody>
    </\${name}>
  );
}
...
export function Demo() {
  return (
    <\${name} defaultValue="documentation">
      <\${name}.List>
        <\${name}.Tab value="documentation">Documentation</\${name}.Tab>
        <\${name}.Tab value="props">Props</\${name}.Tab>
      </\${name}.List>
    </\${name}>
  );
}
...
export function Demo() {
  return (
    <\${name} tone="\${state.tone}" title="\${label}">
...
page = replaceFunction(page, "buildCode", buildCode);
page = replaceFunction(page, "createPropsRows", createPropsRows);
page = replaceFunction(page, "createStylesRows", createStylesRows);
...
const report = [
  "# Real Props Styles Foundation Report",
  "",
  ...
  "## Applied",
  "",
  "- Added component category resolver for documentation metadata.",
  "- Replaced generic code examples with category-aware examples.",
  "- Replaced generic props table with category-aware props tables.",
  "- Replaced generic Styles API table with category-aware selectors, CSS variables and data attributes.",
  "- Covered button-like, field, selection, overlay, surface, data and layout categories first."
].join("\n");

writeText(reportPath, report);

console.log(`Real props/styles foundation completed. Problems: ${problems.length}. Report: ${reportPath}`);

if (problems.length > 0) {
  console.error(report);
  throw new Error("Real props/styles foundation failed.");
}
