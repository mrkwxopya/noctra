...
const reportPath = "button-preview-state-classes-apply-report.md";
...
if (commonStart < 0) {
  throw new Error("commonProps block not found.");
}
...
for (const required of [
  "BUTTON_PREVIEW_INTERACTIVE_VISUAL_STATE_START",
  ".ncd3-topbar{display:none!important}",
  ".ncr-mock-button.ncr-tone-primary",
  ".ncr-mock-button.ncr-variant-outline",
  ".ncr-mock-button.ncr-size-xl",
  ".ncr-mock-button.ncr-radius-full"
]) {
...
if (afterRuntime.includes('className: cx("ncr-mock", `ncr-mock-${kebab(displayName)}`, className)')) {
  problems.push("Old static className block still exists.");
}
...
const report = [
  "# Button Preview State Classes Apply Report",
  "",
  ...
  "## Applied",
  "",
  "- Replaced commonProps className with mockStateClass.",
  "- Added data-variant/data-tone/data-size/data-radius attributes.",
  "- Loading now disables button and renders spinner state.",
  "- Header remains hidden."
].join("\n");

writeText(reportPath, report);

console.log(`Button preview state classes apply completed. Problems: ${problems.length}. Report: ${reportPath}`);

if (problems.length > 0) {
  console.error(report);
  throw new Error("Button preview state classes apply failed.");
}
